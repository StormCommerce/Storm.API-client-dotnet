...
namespace Enferno.Web.StormUtils.Test
{
    ...
    public class DibsCallbackHandlerTest : TestBase
    {
        ...
        public void ProcessValidRequestTest1()
        {
            ...
            var handler = new DibsCallbackHandler(repository);
            var request = new HttpContext(
                new HttpRequest("", application.Url, "status=OK&s_paymentCode=1&s_applicationKey=1"), 
        ...
        public void ProcessRequestWithInvalidParametersTest1()
        {
            ...
            var handler = new DibsCallbackHandler(repository);
            var request = new HttpContext(
                new HttpRequest("", application.Url, "xx=OK"),
                new HttpResponse(new StringWriter()));
